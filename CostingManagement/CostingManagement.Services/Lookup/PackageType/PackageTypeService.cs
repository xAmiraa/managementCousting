...
namespace CostingManagement.Services.Lookup.PackageType
{
    public class PackageTypeService : IPackageTypeService
    {
        ...
        public PackageTypeService(AppDbContext appDbContext,
        ...
        public IResponseDTO SearchPackageTypes(PackageTypeFilterDto filterDto)
        {
            try
            {
                var query = _appDbContext.PackageTypes.Where(x => !x.IsDeleted);
        ...
        public async Task<IResponseDTO> RemovePackageType(int id)
        {
            try
            {
                var packageType = _appDbContext.PackageTypes.FirstOrDefault(x => x.Id == id);
                ...
                _response.IsPassed = true;
                _response.Message = "Package type is removed successfully";
            }
            catch (Exception ex)
            {
                _response.Data = null;
